...
public class PlayerShootingState : PlayerState
{
    ...
    {
        Debug.Log("State: Shooting FireBall");
        player.Combat.StartShootingFireBall();

        player.Combat.ShootFireBall(player.FaceRight);
        player.SetParameter(player.boolShootParameter, true);
    }

    public override void ExitState(PlayerStateController state, PlayerController player)
    {
        player.SetParameter(player.boolShootParameter, false);
    }
    ...
    {
        if (!player.IsShooting && player.IsGrounded)
        {
